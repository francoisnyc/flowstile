...
**Declarative mappings collapse the submission-field plumbing.** `createAndWait` accepts two optional mappings, applied by the SDK in workflow code (no server-side data authority):

- `contextFrom` — project named case variables into the task's `contextData` before creation. Array keeps the name, object renames (`{ creditScore: 'CREDIT_SCORE' }`). The first task of a case (no entity yet) projects nothing rather than failing.
- `persist` — promote an allowlist of submission fields to the case entity on completion. Array keeps the name, object renames (`{ DECISION: 'underwritingDecision' }`).
