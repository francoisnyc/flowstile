...
**Variables: compute in the workflow, persist, project.** Two surfaces hold data and they answer different questions. *Task payloads* (`inputData` / `contextData` / `submissionData`) are the I/O of a single step. The *case entity* (`Case.entity`, optionally validated by `caseEntitySchema`) is the durable, cross-task **variable store** — what later steps, parallel branches, and the case overview read. See `docs/design-decisions.md` → "The Case Entity" for the rationale and concurrency model; the rules an author follows:

- **Calculation lives in the workflow — never in a form field or a server-side expression.** Derived values (a risk tier, a total, a flag) are computed in plain TypeScript and written to the entity. Flowstile has **no calculated-field / formula mechanism by design**: an expression engine would be a second place business logic lives, free to drift from the workflow — the same reason the case plan never gates execution. A live form preview (`qty × price` as the user types) is the only place client-side calc belongs, and it is non-authoritative UX — the real value still returns in `submissionData` and is re-validated server-side.
- **The loop is compute → persist → project.** Compute in workflow code; **persist** durable facts to the entity via `patchFlowstileCaseEntity` — a chosen *allowlist* of fields, not all of `submissionData` (that is the task's transient output; the entity is curated business data); **project** what a human needs into the next task's `contextData`. `getFlowstileCaseEntity` reads it back — most valuable when the data came from another branch, including a `patched()` in-flight branch with no closure over earlier tasks' locals.
- **Projected values are snapshots.** Anything passed into a task's `contextData` / `inputData` is frozen at task creation; if the entity changes later, that task does not update. That is the audit-faithful "what the reviewer saw" — do not reach for `contextData` expecting a live view of a changing variable.
- **Retrieve/assign sequence.** Disjoint-field writes need no version (applied under a row lock; distinct JSON Patch paths never collide). A *same-field* write from concurrent branches is the one case needing optimistic concurrency: read `{ entity, entityVersion }`, recompute, write back with `expectedVersion`, and on a **409** re-read and retry — and that retry loop belongs in workflow code, **not** in the activity's Temporal retry policy (blindly retrying a 409 re-sends the stale version).

`packages/worker/src/loan-origination/workflow.ts` is the worked reference: the automated credit phase computes `creditScore`, the workflow derives `riskTier` from it, both are persisted as case variables and projected into the underwriting and final-decision tasks, and each human outcome is promoted back to the entity. *Planned (Phase 2):* a declarative input/output mapping on the task descriptor — provisionally `contextFrom` (project case variables into the task) and `persist` (allowlist submission fields back to the entity) — to collapse the per-task project/persist boilerplate, applied by the SDK in code so no server-side data authority is introduced. Plumbing only; calculation stays in the workflow.
