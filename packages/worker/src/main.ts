...
createFlowstileWorker({
  process: [orderFulfillmentProcess, loanOriginationProcess, expenseApprovalProcess, vacationLeaveRequestProcess],
  flowstile: {
  ...
  activities: { processPayment, refundPayment, cancelShipment, fetchCreditScore, recordReimbursement, recordLeaveLedger },
}).catch((err) => {
